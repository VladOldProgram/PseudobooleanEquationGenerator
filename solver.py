...
def equation_solution(y, d):
    ...
    if d > 0 and all(y[i]['coef'] < d for i in range(len(y))) and np.sum([y[i]['coef'] for i in range(len(y))]) < d:
        return [list(np.repeat(None, len(y)))]  # вот это плохо, если нет решения выводяться пустые значения
    ...
    for solution in y_solutions:
        ...
        for i in range(len(y)):
            if solution[i] == '-':
                ...
                x_solutions[-1][y[i]['iter']] = int(not solution[i]) if y[i]['inv'] else solution[i]
    return x_solutions
